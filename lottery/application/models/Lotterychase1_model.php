...
//用户购彩模型（自购）
class LotteryChase1_model extends CI_Model{
	const TBL_CHASEUSER = 'chaseuser_1';
	const TBL_LOTTERYCHASE = 'lotterychase_1';
	//const TBL_LOTTERYCHASEENTITY = 'lotterychaseentity';
	#添加追号订单
	public function add_lotterychase($chase,$firstChase){
		//$this->db->trans_start();
		$this->db->insert(self::TBL_CHASEUSER,$chase);
		$this->db->insert(self::TBL_LOTTERYCHASE,$firstChase);
	...
	#获取用户已追号订单列表
	public function get_lotteryChase($chaseID){
	...
	public function get_lotteryChaseList($condition,$num,$offset){
		...
		$this->db->select($sql);
		$this->db->from('lotterychase1 as b');
		$this->db->join('chaseuser1 as a','a.ID= b.chaseID','left');
		$this->db->limit($offset,$num);
		$this->db->where($condition);
