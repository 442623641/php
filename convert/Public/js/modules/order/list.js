	...
	List.prototype = {
		...
		rendData: function(data, target){
			...
			$.each(data, function(index, el){
				tr += '<tr>' +
						'<td><input name="order[]" type="checkbox" value="'+ el.id + '"></td>' + 
						'<td>'+ el.finish_date + '</td><td>' + el.shop +'</td>' +
						'<td>' + el.app + '</td>' + 
						'<td>' + el.shop + '</td>' +
						'<td>' + el.cost + '</td>' + 
						'<td><span>' + el.state_rcs + '</span></td>' + 
						'<td><a href="ddxq.html">查看</a></td>' +
					'</tr>';
			});
		...
		pageBar: function(conf, total, pagesize, url, target){
			var page_num = Math.ceil(total/conf.pageSize),
				...
				pagems = '<div class="fl">共' + total + '条订单信息 每页' + pagesize + '条  共' + page_num + '页  当前'+ curpage + '/' + page_num + '</div>';
