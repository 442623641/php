	...
	List.prototype = {
		...
		rendData: function(data, target){
			...
			$.each(data, function(index, el){
				tr += '<tr class="bs">' + 
						'<td><input name="lists[]" type="checkbox" value=""></td>' + 
						'<td class="aleft"><span>['+ el.comment +']</span>'+ el.comment +
						'</td><td>' + el.finish_date + '&nbsp' + el.finish_time + '</td></tr>';
			});
		...
		pageBar: function(conf, total, pagesize, url, target){
			var page_num = Math.ceil(total/conf.pageSize),
				...
				pagems = '<div class="fl">共' + total + '条信息 每页' + pagesize + '条  共' + page_num + '页  当前'+ curpage + '/' + page_num + '</div>';
