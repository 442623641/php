...
class Common_Controller extends CI_Controller {
	...
	public function __construct() {
		parent::__construct();
		$this->domain = "http://xk.xiangw.com.cn/xk.php";
...
class Home_Controller extends Common_Controller{
	 ...
	 */
	public function __construct() {
		parent::__construct();		$this->set_session('userID','10001');		$this->set_session('user','Leo');		$this->set_session('isLogin',true);		$this->set_session('code','1243');		$this->isLogin=true;
	...
	}	/**	 * POST请求接口	 * @param  [type] $url     接口类名	 * @param  [type] $class     接口类名	 * @param  [type] $function  接口方法�&#65533;	 * @param  [type] $parameter 传递post参数	 * @param  [type] $type      请求类型HTTP, HTTPS	 * @return [type]            请求结果	 */	public function requestWithurl($url,$class, $function, $parameter, $type=CURLPROTO_HTTP) {		if ($type == 'HTTPS') {			$type = CURLPROTO_HTTPS;		}		$url = $url . '/' .$class . '/' . $function;		$timeout = empty($timeout) ? $this->timeout : 10;		try {			$ch = curl_init();			curl_setopt($ch, CURLOPT_URL, $url);			curl_setopt($ch, CURLOPT_PROTOCOLS, $type);			curl_setopt($ch, CURLOPT_POST, 1);	    	curl_setopt($ch, CURLOPT_POSTFIELDS, $parameter);			curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);			curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, $this->timeout);			$result = curl_exec($ch);			curl_close($ch);			$retArr = json_decode($result,true);		} catch (Exception $e) {			$retArr['fail'] = 3015;			$retArr['mess'] = $e->getMessage();		}		return $retArr;	}
}
...
class Admin_Controller extends CI_Controller{
	public function __construct(){
		parent::__construct(); $this->load->switch_themes_off();
		#权限验证
...
class API_Controller extends CI_Controller{
	...
	public function __construct(){
		parent::__construct();
		$this->timeout = 1;
		$this->domain ='http://api.xiangw.com.cn';
		//验证是否有权限调用接�&#65533;
		//$this->_sign();
	}
